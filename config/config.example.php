<?php
declare(strict_types=1);

/**
 * Copy this file to config.php, then configure it for the local XAMPP database.
 * Do not commit environment-specific production credentials.
 */
const APP_NAME = 'Quetta AgriLink';
const APP_ENV = 'development';
const APP_URL = '/quetta-agrilink';
const DB_HOST = '127.0.0.1';
const DB_PORT = '3306';
const DB_NAME = 'quetta_agrilink';
const DB_USER = 'root';
const DB_PASS = '';
const SESSION_IDLE_MINUTES = 60;
const MAX_UPLOAD_BYTES = 5_242_880;
const UPLOAD_STORAGE_PATH = __DIR__ . '/../uploads';
// Set a channel only after it is owned, monitored, and approved for customer use.
const SUPPORT_CHANNEL_MODE = 'unconfigured'; // 'unconfigured', 'email', or 'helpdesk'
const SUPPORT_EMAIL = '';
const SUPPORT_HELPDESK_URL = '';

date_default_timezone_set('Asia/Karachi');
