/** Orchard Ledger preview shell: practical agriculture commerce navigation with a mobile-ready route menu. */
...
export function PreviewLayout({ children }: { children: ReactNode }) {
  ...
  return <div className="agri-shell">
    <header className="agri-header">
      ...
      <div className="agri-nav-actions"><Link className="text-link" href="/sign-in">Sign in</Link><Link className="button button-canopy" href="/sign-in">Create account <ArrowUpRight size={15} /></Link></div>
      ...
      {open && <nav className="mobile-nav" aria-label="Mobile navigation">{navigation.map(([label, href]) => <Link key={href} href={href} onClick={() => setOpen(false)}>{label}</Link>)}<Link href="/sign-in" onClick={() => setOpen(false)}>Open a workspace</Link></nav>}
    </header>
    <main>{children}</main>
