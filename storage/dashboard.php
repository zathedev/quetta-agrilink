...
if ($providerId > 0) {
    ...
    $pending = fetch_one('SELECT COUNT(*) AS count FROM storage_bookings sb JOIN storage_facilities sf ON sf.id=sb.facility_id WHERE sf.provider_id=:provider AND sb.status="requested"', ['provider' => $providerId]) ?? $pending;
    $active = fetch_one('SELECT COUNT(*) AS count FROM storage_bookings sb JOIN storage_facilities sf ON sf.id=sb.facility_id WHERE sf.provider_id=:provider AND sb.status IN("approved","active")', ['provider' => $providerId]) ?? $active;
...
?>
<?php if ($providerId === 0): ?><section class="workspace-focus"><span>Account setup pending</span><h2>No facility profile has been added for this account.</h2><p>This retained development credential has no fictional storage operation. Add verified organisation details and a real facility before using the account for live capacity or booking work.</p></section><?php endif; ?>
