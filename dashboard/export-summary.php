...
audit_log((int) $user['id'], 'dashboard_summary_exported', 'dashboard_summary', null, ['role' => $user['role_slug'], 'period_start' => $window['from_input'], 'period_end' => $window['to_input']]);
header('Content-Type: text/csv; charset=utf-8'); header('Content-Disposition: attachment; filename="quetta-agrilink-summary-' . e($user['role_slug']) . '-' . $window['from_input'] . '-to-' . $window['to_input'] . '.csv"'); header('Cache-Control: no-store, private');
