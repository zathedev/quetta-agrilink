...
if ($providerId > 0) {
    ...
    $pending = fetch_one('SELECT COUNT(*) AS count FROM transport_requests WHERE provider_id=:provider AND status="requested"', ['provider' => $providerId]) ?? $pending;
    $active = fetch_one('SELECT COUNT(*) AS count FROM transport_requests WHERE provider_id=:provider AND status IN("accepted","driver_assigned","pickup_scheduled","picked_up","in_transit")', ['provider' => $providerId]) ?? $active;
    $delivered = fetch_one('SELECT COUNT(*) AS count FROM transport_requests WHERE provider_id=:provider AND status="delivered"', ['provider' => $providerId]) ?? $delivered;
...
?>
<?php if ($providerId === 0): ?><section class="workspace-focus"><span>Account setup pending</span><h2>No transport provider profile has been added for this account.</h2><p>This retained development credential has no fictional fleet or dispatch operation. Add verified organisation details and real vehicles before using the account for live transport work.</p></section><?php endif; ?>
