...
| Dashboard export audit | Only an administrator can filter dashboard-summary export audit events by account, role, export-event date range, or selected summary period. Quick export-event presets cover today, the last 7 days, and this month while retaining active filters. The table has bounded filter-preserving pages and allowlisted column sorting, and its filtered CSV contains only the displayed accountability columns. |
