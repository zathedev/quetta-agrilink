...
function listing_card_html(array $listing, int $index = 0): string
{
    ...
    $tone = ['leaf', 'grape', 'apricot', 'pomegranate'][$index % 4];
    ob_start(); ?>
    <article class="listing-card listing-tone-<?= e($tone) ?> <?= $image === '' ? 'listing-card-no-media' : 'listing-card-has-media' ?>"><?php if ($image !== ''): ?><div class="listing-card-media" style="background-image:url('<?= e($image) ?>')"></div><?php endif; ?><div class="listing-card-body"><div><div class="listing-card-top"><span class="badge">Grade <?= e($listing['grade']) ?></span><h2><?= e($listing['title']) ?></h2><div class="commodity-mark <?= e($tone) ?>" aria-hidden="true"></div></div><p class="origin-stamp"><?= e($listing['district']) ?>, Balochistan · <?= e($listing['farmer_name']) ?></p></div><div class="listing-data"><div><span>Available</span><strong><?= e((string) $listing['quantity_available']) ?> <?= e($listing['unit']) ?></strong></div><div><span>Expected price</span><strong>Rs. <?= number_format((float) $listing['expected_price'], 0) ?>/<?= e($listing['unit']) ?></strong></div><div><span>Minimum order</span><strong><?= e((string) $listing['minimum_order_quantity']) ?> <?= e($listing['unit']) ?></strong></div><div><span>Harvest</span><strong><?= $listing['harvest_date'] ? e(date('j M Y', strtotime($listing['harvest_date']))) : 'Not stated' ?></strong></div></div><div class="listing-card-actions"><span class="listing-category"><?= e($listing['category_name']) ?></span><a class="button button-quiet" href="<?= e(app_url('marketplace/listing.php?id=' . (int) $listing['id'])) ?>">View listing</a></div></div></article>
